...
public class StorePage {
    ...
    public List<HoveredCardItem> printHoveredCardData(List<WebElement> searchResults) {
        WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));

        List<HoveredCardItem> hoveredItems = new ArrayList<>();
        ...
        for (int i = 0; i < 3; i++) {
            ...
            actions.moveToElement(searchItem).perform();

            wait.until(ExpectedConditions.visibilityOfElementLocated(itemScreenType));
            ...
            HoveredCardItem item = new HoveredCardItem();
            item.setDeviceName(productName);
            item.setDevicePrice(productPrice);
            item.setDeviceScreenType(productScreenType);
